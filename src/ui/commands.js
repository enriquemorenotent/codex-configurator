...
const readCommand = (value) => String(value || '').trim().toLowerCase();
...
const runCommand = (context, rawInput) => {
  const commandInput = typeof rawInput === 'string' ? rawInput : readCommandInput(context);
  ...
  if (command === 'help') {
    context.setShowHelp((previous) => !previous);
...
const navigateToParentPath = ({
  ...
  const parentNode = context.getNodeAtPath(snapshot.ok ? snapshot.data : {}, backTargetPath);
  ...
  const parentSelected = parentRows.length === 0 ? 0 : context.clamp(savedIndex, 0, parentRows.length - 1);
  ...
  context.setStateBatch({
    ...
    scrollOffset: context.clamp(parentSelected, 0, Math.max(0, parentRows.length - parentViewportHeight)),
    editMode: null,
    editError: '',
  });
...
export const executeInputCommand = ({
  input,
  key,
  context,
}) => {
  ...
  if (isCapturingCommand) {
    ...
    if (
      isCommandNavigationCaptureKey(input, key)
    ) {
      return true;
    }
  ...
  if (isFileSwitchMode) {
    if (key.upArrow) {
      context.setFileSwitchIndex((previous) => context.clamp(previous - 1, 0, configFileCatalog.length - 1));
      return true;
    }
    ...
    if (isPageUpKey(input, key)) {
      context.setFileSwitchIndex((previous) =>
        context.clamp(previous - listViewportHeight, 0, configFileCatalog.length - 1)
      );
      return true;
    }
    ...
    if (isEndKey(input, key)) {
      context.setFileSwitchIndex(Math.max(0, configFileCatalog.length - 1));
      return true;
    }
    ...
    if (key.escape || isBackspaceKey(input, key) || key.leftArrow) {
      context.setStateBatch({
        ...
        fileSwitchIndex: Math.max(0, configFileCatalog.findIndex((file) => file.id === context.activeConfigFileId)),
        editError: '',
      });
      return true;
    }
  ...
  if (editMode) {
    ...
    if (key.upArrow) {
      context.setEditMode((previous) => ({
        ...previous,
        selectedOptionIndex: context.clamp(previous.selectedOptionIndex - 1, 0, previous.options.length - 1),
      }));
      return true;
    }
  ...
  if (key.return && rows[safeSelected]) {
    ...
    const options = context.getConfigOptions(targetPath, target.key, target.value, target.kind) || [];
